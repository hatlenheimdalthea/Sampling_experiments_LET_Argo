...
def import_member_data(ensemble_dir_head, ens, member, dates, xco2_path="/local/data/artemis/simulations/LET/CESM/member_001/XCO2_1D_mon_CESM001_native_198201-201701.nc"):
    ...
    del tmp2
    #################################################################
    # SOCAT AND ARGO MASKS #
    ###############################################################################
    argo_path = "/data/artemis/workspace/theimdal/GO-BGC/masks/argo_mask_500_random_2000_2016.nc"
    
    inputs['argo_mask'] = xr.open_dataset(argo_path).sail_mask
    ...
    DS = xr.merge([inputs['sss'], inputs['sst'], inputs['mld'], inputs['chl'], inputs['pCO2_DIC'], inputs['pCO2'], inputs['socat_mask'], inputs['argo_mask'],
